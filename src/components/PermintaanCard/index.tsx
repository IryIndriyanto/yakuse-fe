  ...
  return (
    <div className="w-full h-auto p-7 md:p-6 space-x-4 md:space-x-3 flex flex-row items-center">
      ...
      <div className="w-auto h-auto space-y-1 flex flex-col justify-center">
        <h2 className="font-normal text-5xl md:text-3xl text-b-two">
          {username}
        </h2>
  ...
  return (
    <div className="w-full h-auto bg-w-two rounded-xl">
      <ProfilePelanggan
        ...
        userProfile={
          item.user_info.user_profile_url || "/default-gray-photo.webp"
        }
      />
      <div className="w-full h-auto pb-9 px-9 space-y-5 md:space-y-3">
        <div>
          <p className="font-normal text-5xl md:text-3xl text-black">
            {item.title}
          </p>
          <p className="font-thin text-[14px] md:text-xs text-black">date</p>
        </div>
        <p className="font-normal text-lg md:text-[16px] text-b-two">
          {item.description}
        </p>
        <div className="flex flex-row justify-between gap-3">
          ...
          <p
            // key={index}
            className="cursor-pointer hover:bg-[#ff873c] bg-[#FD5F00] text-w-two w-fit h-fit font-normal text-[16px] md:text-[14px] border-[1px] border-w-two rounded-full py-2 px-4"
          >
            Kontak
          </p>
          {/* ))} */}
