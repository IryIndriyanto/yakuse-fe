...
export function ProfilePelanggan({ username, userProfile, onClick }: userDetailProps) {
  return (
    <div className="w-full h-auto p-7 md:p-6 space-x-4 md:space-x-3 flex flex-row items-center" onClick={onClick}>
      <Image
        src={userProfile}
        alt="profile"
        width={60}
    ...
    <div className="w-full h-auto bg-w-two rounded-xl">
      ...
      <div className="w-full h-auto pb-9 px-9 space-y-5 md:space-y-3">
        <div>
          ...
          <p className="font-thin text-[14px] md:text-xs text-black">{item.created_at}</p>
        </div>
        ...
        <div className="flex flex-row flex-wrap gap-3">
          ...
          <p
            // key={index}
            className="w-fit h-fit font-normal text-[16px] md:text-[14px] text-b-two py-2 px-4"
          >
            #{item.category.name}
          </p>
          {/* ))} */}
